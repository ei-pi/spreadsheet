/*
    The code below is a copy of the code found in the corresponding file in the original project.
    The only modification has been to replace all the import statements with a single import pointing to
    the relevant import aggregation file
*/

import { type ItemDefinition, ItemType, type ReferenceTo, type AmmoDefinition, type BaseBulletDefinition, FireMode, v, Vector, mergeDeep } from "./imports.js";
...
export const Guns: GunDefinition[] = GunsRaw.map(e => {
    ...
    //! the two lines below are backported from a fix after v0.14.0
    // @ts-expect-error init code
    delete e.dual;
    // @ts-expect-error init code
